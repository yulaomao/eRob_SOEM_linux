...
#define POSITION_STABILITY_THRESHOLD 1000  // Allowed position fluctuation range
...
//##################################################################################################
   ...
   //set_thread_affinity(*thread2,1);
    ...
    if (ec_slave[0].state == EC_STATE_OPERATIONAL) {
        ...
        uint16_t previous_tenth_bit = 1;  // 添加这行来跟踪上一个状态
...
*/
            ...
            if (step > 800) {
                ...
                int current_target = target_position;
                target_position_updated = false;
                pthread_mutex_unlock(&target_position_mutex);

                if (current_target != 0) {
                    // Update position buffer
                    position_buffer[buffer_index] = current_target;
                    buffer_index = (buffer_index + 1) % POSITION_BUFFER_SIZE;
                    
                    // Check if position is stable
                    position_stable = true;
                    for (int i = 0; i < POSITION_BUFFER_SIZE - 1; i++) {
                        if (abs(position_buffer[i] - position_buffer[i+1]) > POSITION_STABILITY_THRESHOLD*2) {
                            position_stable = false;
                            break;
                        }
                    }
                    
                    // Only execute motion when position is stable
                    if (position_stable) {
                        output_data[0] = current_target & 0xFFFF;          // Lower 16 bits of target position
                        output_data[1] = (current_target >> 16) & 0xFFFF;  // Upper 16 bits of target position
                        output_data[2] = 0x00;
                        output_data[3] = 0x00;
                        output_data[4] = 0x1F;
                        
                        // Maintain existing rotation logic
                        if (previous_tenth_bit == 0 && tenth_bit == 1 && turn_flag == 0) {
                            turn_flag = 1;
                            output_data[3] = 0x00;
                            output_data[4] = 0x0F;
                            previous_position = position_actual_value;
                        }
                        if (turn_flag == 1) {
                            output_data[3] = 0x00;
                            output_data[4] = 0x0F;
                        ...
                        // Reset motion parameters when target is reached
                        if (turn_flag == 1 && abs(current_target-previous_position) > 1000) {
                            ec_SDOwrite(1, 0x6081, 0x00, FALSE, sizeof(Profile_velocity), &Profile_velocity, EC_TIMEOUTSAFE);
                            ec_SDOwrite(1, 0x6083, 0x00, FALSE, sizeof(Profile_acceleration), &Profile_acceleration, EC_TIMEOUTSAFE);
                            ec_SDOwrite(1, 0x6084, 0x00, FALSE, sizeof(Profile_deceleration), &Profile_deceleration, EC_TIMEOUTSAFE);
                            turn_flag = 0;
                        }  
                            
                        printf("turn_flag: %d  , sub_err : %d",turn_flag,abs(current_target-previous_position)); 
                        previous_tenth_bit = tenth_bit;
                    } else {
                        // Maintain current state when position is unstable
                        output_data[3] = 0x00;
                        output_data[4] = 0x0F;
                    }
                }
